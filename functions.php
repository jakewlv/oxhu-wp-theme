...
function oxhu_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'oxhu' ),
		)
	);
...
function oxhu_scripts() {
	wp_enqueue_style( 'oxhu-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'oxhu-style', 'rtl', 'replace' );

	wp_enqueue_script( 'oxhu-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'oxhu_scripts' );
